<template>
  ...
  <aside class="drawer" :class="{ open }" aria-label="سلّة المشتريات">
    ...
    <div class="drawer-foot" v-if="cart.list.length">
      <!-- free-delivery progress nudge (threshold set by the admin). Same component
           as the band under the header, so the two can't quote different numbers. -->
      <FreeDeliveryBar variant="panel" />
